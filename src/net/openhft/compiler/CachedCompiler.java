...
public class CachedCompiler {
	...
	public Class loadFromJava(@NotNull String className, @NotNull String javaCode, @Nullable URL... urls)
		...
		if (urls != null && urls.length > 0) {
			...
			return loadFromJava(urlClassLoader, className, javaCode, DEFAULT_WRITER);
		} else {
	...
	public Class loadFromJava(@NotNull ClassLoader classLoader, @NotNull String className, @NotNull String javaCode)
			throws ClassNotFoundException {
	...
	Map<String, byte[]> compileFromJava(@NotNull String className, @NotNull String javaCode,
		...
		// custom code to generate the byte code with parameter info
		Iterable<String> options = Arrays.asList(new String[] { "-parameters" });
		boolean ok = CompilerUtils.s_compiler
				.getTask(writer, CompilerUtils.s_fileManager, new DiagnosticListener<JavaFileObject>() {
					@Override
